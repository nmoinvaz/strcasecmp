...
int strcasecmp_fast(const char *s1, const char *s2) {
    unsigned char c1, c2;
    ...
    do {
        ...
        c2 = *s2++;
        if (c1 != c2) {
            c1 = tolowermap[(unsigned char) c1];
            c2 = tolowermap[(unsigned char) c2];
            if (c1 != c2)
                return (int)c1 - (int)c2;
        }
    } while (c1 != 0);
...
static void strcasecmp_fast_bench(benchmark::State &state) {
    int r = state.range(0);
    while (state.KeepRunning()) {
        int len = strcasecmp_fast(string1, string2[r]);
        // EXPECT_EQ(len, sizeof(string1) - 1);
        benchmark::DoNotOptimize(len);
    }
}
BENCHMARK(strcasecmp_fast_bench)->Arg(0)->ArgName("100% same");
BENCHMARK(strcasecmp_fast_bench)->Arg(1)->ArgName("83% same");
BENCHMARK(strcasecmp_fast_bench)->Arg(2)->ArgName("75% same");
BENCHMARK(strcasecmp_fast_bench)->Arg(3)->ArgName("66% same");
BENCHMARK(strcasecmp_fast_bench)->Arg(4)->ArgName("50% same");
BENCHMARK(strcasecmp_fast_bench)->Arg(5)->ArgName("33% same");
BENCHMARK(strcasecmp_fast_bench)->Arg(6)->ArgName("25% same");
BENCHMARK(strcasecmp_fast_bench)->Arg(7)->ArgName("15% same");
BENCHMARK(strcasecmp_fast_bench)->Arg(8)->ArgName("0% same");
